...
for mode in ["Debug","ReleaseSafe"]:
    label="final-write-"+mode.lower()
    run(label,["build","test","-Dtest-group=write","-Doptimize="+mode,"-Dfault-injection=true","-Dinstall-tests=true","--prefix",str(S/("out-"+label)),"--summary","all"],"out-"+label)
for mode in ["Debug","ReleaseSafe"]:
    label="regression-all-"+mode.lower()
    run(label,["build","test","-Doptimize="+mode,"-Dinstall-tests=true","--prefix",str(S/("out-"+label)),"--summary","all"],"out-"+label)
run("verify-contracts",["build","verify-contracts","--summary","all"])
run("build-runtime",["build","-Doptimize=ReleaseSafe","--prefix",str(S/"out-runtime"),"--summary","all"],"out-runtime")
