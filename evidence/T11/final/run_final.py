...
S=pathlib.Path("/workspace/scratch/3f4ab65dfb82/state/T11/current")
BASE=S.parent
W=pathlib.Path("/workspace/scratch/3f4ab65dfb82/ZCR-T11")
Z="/workspace/scratch/3f4ab65dfb82/toolchain/ziglang/zig"
env=os.environ.copy()
for key,value in [("ZIG_GLOBAL_CACHE_DIR","zig-global-cache"),("ZIG_LOCAL_CACHE_DIR","zig-local-cache"),("TMPDIR","tmp")]: env[key]=str(BASE/value)
...
for mode in ["Debug","ReleaseSafe"]:
    for group in ["write", "isolation", "fs", "io"]:
        label="final-"+group+"-"+mode.lower()
        run(label,["build","test","-Dtest-group="+group,"-Doptimize="+mode,"-Dfault-injection=true","-Dinstall-tests=true","--prefix",str(S/("out-"+label)),"--summary","all"],"out-"+label)
for mode in ["Debug","ReleaseSafe"]:
    ...
    run(label+"-run",[],executable=str(binary),cwd=BASE/"zig-local-cache")
...
run("runtime-gate-run",[],executable=str(S/"runtime-gate"),cwd=BASE/"tmp")
