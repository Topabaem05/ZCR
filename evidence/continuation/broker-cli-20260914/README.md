...
**Date:** 2026-09-14 · **Host:** Apple M2 8 CPU, macOS 26.6.2, Zig 0.16.0 · **Base:** `961a686` · **Source:** `5716cea` (tree `3ca68fc6…`) · **Task:** T01 integrator (docs/18 row 2, slice C)
...
| Test | Failure |
|---|---|
| `BR-004 zcr broker serve and mcp --broker take the token from an inherited fd and serve reads only` | line 147: `const a = f.arena.allocator();`  |
| `BR-004 zcr mcp --broker without a running broker refuses clearly and starts nothing` | line 205: `// A second bridge finds no broker, is refused clearly, and starts nothing.` expected 69, found 64 |
| `BR-004 zcr broker serve refuses a token on argv and a policy without broker_allowed` | line 216: `try expectExit(&again, 69);`  |
...
Review on PR #9 found that a first bridge's disconnect left the broker listening for bridges it could only refuse. The main test was extended: after the bridge exits, the broker must stop by itself with "grant ended" and remove its socket, and a second bridge must be refused with the standalone hint. A new test keeps the stdin-close stop covered. RED before the fix: the bounded wait for grant ended timed out at line None ([log](logs/red2-grant-ended-debug.log)); the stdin-close test already passed. GREEN after the fix: 6/6 steps succeeded; 4/4 tests passed; 0 leaks ([log](logs/green2-grant-ended-debug.log)). Earlier mutant runs against the first version are kept as `logs/mutant-*-v1.log`.

| Mutant | Change | Result |
|---|---|---|
| `ignore_broker_allowed` | broker role no longer requires broker_allowed | FAIL `BR-004 zcr broker serve refuses a token on argv and a policy without broker_allowed` at line 284: `try t.expect(std.mem.indexOf(u8, try readAll(a, broker.stderr.?), "refused") != null);` ([log](logs/mutant-ignore_broker_allowed.log)) |
| `bridge_zero_token` | bridge sends an all-zero token | FAIL `BR-004 zcr broker serve and mcp --broker take the token from an inherited fd and serve reads only` at line 194: `try t.expect(std.mem.indexOf(u8, responses, "2025-11-25") != null);` ([log](logs/mutant-bridge_zero_token.log)) |
| `watcher_ignores_grant_end` | broker keeps listening after its only grant's binding ends | FAIL `BR-004 zcr broker serve and mcp --broker take the token from an inherited fd and serve reads only` at line 201: `try t.expect(std.mem.indexOf(u8, try readAllWithin(a, broker.stderr.?, 10_000), "grant ended") != null);` ([log](logs/mutant-watcher_ignores_grant_end.log)) |

## Records (`zcr-evidence/1`, verified after recording)

The runs started with no other zig process, no other evidence run and a 1-minute load average below the CPU count, at `2026-09-14T06:43:25Z start 15:43  up 19:09, 1 user, load averages: 2.42 20.63 39.93`. Other applications on the host were not stopped, and the load rose during the run.

| Record | Run | Binary | Result |
|---|---|---|---|
| [quiet-broker-debug](records/quiet-broker-debug.json) | broker group Debug (T01-broker-cli-test binary) | `45c2a4e4…` | exit 0; 10/10 steps succeeded; 32/32 tests passed; 0 leaks ([log](logs/quiet-broker-debug.log)) |
| [quiet-broker-releasesafe](records/quiet-broker-releasesafe.json) | broker group ReleaseSafe (T01-broker-cli-test binary) | `93168649…` | exit 0; 10/10 steps succeeded; 32/32 tests passed; 0 leaks ([log](logs/quiet-broker-releasesafe.log)) |
...
| [quiet-mcp-debug](records/quiet-mcp-debug.json) | mcp group Debug including MC-005 launch tests (T08-launch-test binary) | `c5d164db…` | exit 0; 8/8 steps succeeded; 29/29 tests passed; 0 leaks ([log](logs/quiet-mcp-debug.log)) |

[records/runs.json](records/runs.json) binds each record to its log digest, `source_config_sha256` `be579c07…` ([source-config.json](records/source-config.json)) and `corpus_sha256` `c0fd2be3…` ([corpus.json](records/corpus.json)).
