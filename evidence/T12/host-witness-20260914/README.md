...
**Date:** 2026-09-14 · **Host:** Apple M2 8 CPU, macOS 26.6.2, Zig 0.16.0 · **Base:** `9c94517` · **Source:** `0a8627d` (tree `e753bf93…`)
...
## Change (`0a8627d`: `src/storage/recovery.zig`, `tests/t12_test.zig`)
...
- **Tests:** `WR-008 host witness grants once and refuses a replaced temp name after unlink` and `WR-008 host witness accepts an applied publication and refuses a foreign target`.
...
|---|---|---|
| `no_temp_name_check` | accept any entry at the temp name | FAIL `WR-008 host witness grants once and refuses a replaced temp name after unlink` ([log](logs/mutant-no_temp_name_check.log)) |
| `no_one_use` | drop the one-use guard on validate (quiet rerun) | FAIL `WR-008 host witness grants once and refuses a replaced temp name after unlink` ([log](logs/mutant-no_one_use-quiet.log)) |
| `leaf_original_only` | accept only the original at the target, not the renamed temp (quiet rerun) | FAIL `WR-008 host witness accepts an applied publication and refuses a foreign target` ([log](logs/mutant-leaf_original_only-quiet.log)) |

The first runs of two mutants shared the CPU with other builds. Each failed its intended test, but `no_one_use` also failed 3 older WR-008 tests ([log](logs/mutant-no_one_use.log)); `leaf_original_only` also failed 3 older WR-008 tests ([log](logs/mutant-leaf_original_only.log)). Those extra failures went through discovery git timeouts (`process.run` in `identity.discoverWith`), the load condition recorded in `evidence/T10/git-timeout-20260914`. The quiet reruns above are the mutant results.

## Records (`zcr-evidence/1`, verified after recording)

| Record | Run | Binary | Result |
|---|---|---|---|
| [loaded-write-debug-t12](records/loaded-write-debug-t12.json) | write group Debug; load average about 220 from other applications, run stopped after Debug | `1796f953…` | exit 1; 8/11 steps succeeded (2 failed); 64/70 tests passed (6 failed); 12 leaks ([log](logs/loaded-write-debug.log)) |
| [quiet-write-debug-t12](records/quiet-write-debug-t12.json) | write group Debug; started at load 7.87, load rose past 38 during the run | `1796f953…` | exit 1; 8/11 steps succeeded (2 failed); 68/70 tests passed (2 failed); 3 leaks ([log](logs/quiet-write-debug.log)) |
| [quiet-write-releasesafe-t12](records/quiet-write-releasesafe-t12.json) | write group ReleaseSafe; same session | `6e489b82…` | exit 0; 11/11 steps succeeded; 70/70 tests passed; 0 leaks ([log](logs/quiet-write-releasesafe.log)) |
| [quiet2-write-debug-t12](records/quiet2-write-debug-t12.json) | write group Debug; started at load 5.70, a concurrent local build then raised load to 45-60 | `1796f953…` | exit 1; 8/11 steps succeeded (2 failed); 68/70 tests passed (2 failed); 4 leaks ([log](logs/quiet2-write-debug.log)) |
| [quiet3-write-debug-t12](records/quiet3-write-debug-t12.json) | write group Debug; started at load 4.65 | `1796f953…` | exit 0; 11/11 steps succeeded; 70/70 tests passed; 0 leaks ([log](logs/quiet3-write-debug.log)) |
...
[records/runs.json](records/runs.json) binds each record to its log digest, `source_config_sha256` `27f3af4e…` ([source-config.json](records/source-config.json)) and `corpus_sha256` `e3db5865…` ([corpus.json](records/corpus.json)).
