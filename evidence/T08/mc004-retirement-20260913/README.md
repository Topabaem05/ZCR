...
- **Process deviation (not compliance):** the manifest was issued when these records were made. The development commits `bd54afe` and `ae5e4a4` were made in a clean per-task worktree with separate caches and TMP, but **not under a pre-issued manifest**. The manifest, preflight and scope report authenticate the PASS re-recording and show every change since `9226555` is inside the write paths. They do not show that the test edits were made under an approved manifest, and S01 is not claimed for them (`handoff.json` `process_deviations`). Replaying the edits from a pre-issued manifest would produce the same `tests/t08_test.zig` diff, so the decision is left to the repository owner at merge.
